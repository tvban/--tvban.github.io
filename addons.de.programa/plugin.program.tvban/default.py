		...
		if len(match) >= 1:
			if SEPERATE == 'true':
				...
				if count15 > 0:
					...
					addFile('[B]%s [COLOR red]Gracias por Instalar GTKing Prime (%s)[/COLOR][/B]' % (state, count15), 'togglesetting',  'show15', themeit=THEME2)
					if SHOW15 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
				...
				if count17 > 0:
					...
					addFile('[B]%s Otros (%s)[/B]' % (state, count17), 'togglesetting',  'show17', themeit=THEME3)
					if SHOW17 == 'false':
						for name, version, url, gui, kodi, theme, icon, fanart, adult, description in match:
							if not SHOWADULT == 'true' and adult.lower() == 'yes': continue
							if not DEVELOPER == 'true' and wiz.strTest(name): continue
...
def viewBuild(name):
    ...
    for version, url, gui, kodi, themefile, icon, fanart, preview, adult, info, description in match:
        ...
        if warning == True:
            addFile('[COLOR dodgerblue][B]INSTALAR PARCHE[/B][/COLOR]', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
